...
import { Sparkles, Globe, Loader2, Plus, Minus, Zap, Crown } from 'lucide-react';
import { PaymentModal } from './PaymentModal';
import { InteractiveGlobe } from './InteractiveGlobe';
import { LeaderboardItemData } from './LeaderboardItem';

interface HeroBentoProps {
  topItem?: LeaderboardItemData;
  topBid: number;
  ...
  return (
    <section className="w-full max-w-6xl mx-auto mb-10">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-stretch">
        ...
        {/* CARD 1 (LEFT 50%): 3D INTERACTIVE LIVE GLOBE */}
        <InteractiveGlobe onlineVisitors={onlineVisitors} />

        {/* CARD 2 (RIGHT 50%): QUICK OUTBID & TAKEOVER CONSOLE */}
        <div className="rounded-3xl glass-panel p-6 sm:p-7 flex flex-col justify-between h-full">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold uppercase tracking-wider text-zinc-300 flex items-center gap-1.5">
                <Zap className="size-3.5 text-amber-400 fill-amber-400" />
                <span>Outbid & Take the Lead</span>
              </span>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
              ...
              <button
                ...
                className="px-3 py-2 rounded-2xl border border-white/10 bg-zinc-900/80 hover:bg-zinc-800 text-xs font-semibold text-zinc-300 transition"
              >
                +$100
              </button>
              <button
                ...
                className="px-3 py-2 rounded-2xl border border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/20 text-xs font-bold text-amber-400 transition"
              >
                +$1k 🔥
              </button>
            </div>
          ...
          <div className="mt-4 pt-3.5 border-t border-white/[0.07] flex items-center justify-between text-xs">
